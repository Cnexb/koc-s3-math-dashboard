...
def compact_salary_table() -> VGroup:
    ...
    return VGroup(panel, heading, rule, divider, years, values).move_to([-4.25, -0.05, 0])
...
class SalaryCheckpoint(PctSlide):
    ...
    def construct(self):
        ...
        years.move_to([-4.35, -1.05, 0])
        values.move_to([1.25, -1.05, 0])
        divider = Line([-3.0, -2.2, 0], [-3.0, 0.1, 0], color=MUTED)
        table_rule = Line([-5.6, 0.05, 0], [5.8, 0.05, 0], color=MUTED)
        ...
        self.play(FadeIn(table_1, shift=RIGHT * 0.12))
        self.play(Write(step_1))
        self.play(Write(factor_1))
        self.play(Write(working_1))
        self.play(FadeIn(result_1))
        self.next_slide()
        self.play(FadeOut(VGroup(prompt_1, table_1, step_1, factor_1, working_1, result_1)))
        ...
        table_2 = compact_salary_table()

        self.play(FadeIn(prompt_2, shift=DOWN * 0.1))
        self.play(FadeIn(table_2, shift=RIGHT * 0.12))
        self.play(Write(carried))
        self.play(Write(step_2))
        self.play(Write(factor_2))
        self.play(Write(working_2))
        self.next_slide()
        self.play(FadeOut(VGroup(prompt_2, table_2, carried, step_2, factor_2, working_2)))

        # Answer page: present the complete reverse calculation together.
        ...
        self.play(FadeIn(table_3, shift=RIGHT * 0.12))
        ...
        self.play(FadeOut(VGroup(
            prompt_3, table_3, reverse, check, answer_label, answer
        )))

        # Final page: recall the basic percentage-change formula (as on the sheet).
        recall = Tex("Recall:", color=INK, font_size=40).move_to([-5.0, 1.85, 0])
        formula = MathTex(
