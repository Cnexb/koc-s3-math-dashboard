/* Factorization L02 quiz — same runner as L01, separate DOM ids and tracker meta. */
(function () {
  "use strict";

  const QUIZ = (window.MATH_QUIZ_BANKS || {})['factorization-l02'];
  if (!QUIZ) return;
  ...
  function initQuiz() {
    const root = document.getElementById("quiz-root-l02");
    const progressWrap = document.getElementById("quiz-progress-wrap-l02");
    const progressLabel = document.getElementById("quiz-progress-label-l02");
    const progressFill = document.getElementById("quiz-progress-fill-l02");
    const progressOk = document.getElementById("quiz-progress-ok-l02");
    const progressBad = document.getElementById("quiz-progress-bad-l02");
    const backBtn = document.getElementById("quiz-back-l02");
    const nextBtn = document.getElementById("quiz-next-l02");
    ...
    function buildMc(q, reviewMode) {
      ...
      q.choices.forEach((tex, i) => {
        ...
        inp.type = "radio";
        inp.name = reviewMode ? "review-l02-q-" + q.id : "l02-q-" + q.id;
        inp.value = String(i);
        ...
        try {
          if (window.MathQuizTracker) {
            window.MathQuizTracker.reportQuiz(
              window.MathQuizTracker.QUIZ_META['factorization-l02'],
              QUIZ,
              state.answers,
              function (q, answers) {
                var userAnswerIdx = answers[q.id];
                return checkQuestion(q, answers);
              }
            );
          }
        } catch(_) {}
