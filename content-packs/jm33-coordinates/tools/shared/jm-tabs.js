(function () {
  ...
  window.initJmTabs = function () {
    ...
    var tab = new URLSearchParams(location.search).get("tab") || hash;
    var known = {};
    document.querySelectorAll(".jm-tab").forEach(function (btn) {
      known[btn.dataset.tab] = true;
    });
    if (tab && known[tab]) showTab(tab);
    else renderKatex(document.body);

    if (new URLSearchParams(location.search).get("embed") === "1" || window.self !== window.top) {
